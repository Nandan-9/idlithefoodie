...
import { fetchInstantFeed } from "@/lib/api";
import {
  applyOverrides,
  reconcileOverride,
  subscribeOverrides,
...
export function useInstantFeed() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const posts = await fetchInstantFeed();
      ...
      setState((s) => ({
        ...s,
        loading: false,
        error: "Could not load instant feed. Pull down to retry.",
      }));
    }
  }, []);
